...
<body class="bg-light">
  <div class="container py-4">
    ...
    <div class="card mb-3">
      <div class="card-body">
        <form class="row g-2" method="GET" action="{{ route('admin.history') }}">
          <div class="col-md-3">
              ...
              @if(isset($tableReady) && $tableReady)
                @forelse($histories as $i => $h)
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-sm btn-outline-danger" onclick="confirmSoftDelete({{ $h->id }})" title="Move to Trash">
                ...
                @empty
                  <tr>
                    <td colspan="10" class="text-center py-4 text-muted">No history records found.</td>
                  </tr>
                @endforelse
              @else
                <tr>
                  <td colspan="10" class="text-center py-4 text-muted">Table not available yet. Please run migrations.</td>
                </tr>
              @endif
            </tbody>
          </table>
  ...
  <script>
    ...
    function confirmSoftDelete(id) {
      Swal.fire({
        title: 'Move to Trash?',
        text: 'This record will be soft deleted and can be permanently removed later.',
        icon: 'warning',
