...
<head>
  <meta charset="UTF-8">
  <title>Edit Part-time Timesheet</title>
  <!-- Bootstrap 5 CSS -->
...
<body>
      ...
      <div class="mb-3">
        ...
        <div class="days-selector mb-2">
          <div class="row">
            @php
              $existingDays = [];
              if (is_array($timesheet->days)) {
                $existingDays = array_map('intval', $timesheet->days);
              } elseif (is_string($timesheet->days)) {
                $decoded = json_decode($timesheet->days, true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                  $existingDays = array_map('intval', $decoded);
                } else {
                  $existingDays = array_map('intval', array_filter(explode(',', $timesheet->days)));
                }
              }
            @endphp
            @for($i = 1; $i <= 15; $i++)
              <div class="col-md-3 col-sm-4 col-6 mb-2">
                <div class="form-check">
                  <input class="form-check-input day-checkbox" type="checkbox" value="{{ $i }}" id="day{{ $i }}" {{ in_array($i, $existingDays) ? 'checked' : '' }}>
  ...
  <script>
    ...
    // Initial calculation and days field update
    calculateTotal();
    updateDaysField();
  </script>
