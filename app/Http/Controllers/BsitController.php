...
class BsitController extends Controller
{
    ...
    {
        // Fetch only BSIT department timesheets
        $fulltimeTimesheets = FulltimeTimesheet::where('department', 'BSIT')->get();
        $parttimeTimesheets = ParttimeTimesheet::where('department', 'BSIT')->get();
        ...
        foreach ($fulltimeTimesheets as $timesheet) {
            $attendanceRecords->push([
                ...
                'department' => $timesheet->department ?? 'BSIT',
        ...
        return view('bsit.index', compact('attendanceRecords'));
    }
